...
export const OPChefOrderCompleted = () => {
    ...
    function fnCompletedSelectedButton() {
        ...
        const newContent = tbl_menu.current.tableData.map(item => ({
            ...
            table_value: [
                ...
                {
                    t_key: 2,
                    t_value: item.table_value[2].t_value === "New" ? "Processing" : "Completed"
                },
            ]
        }));
        const companyName = getAppStoreData.companyName;
        socketRef.current.emit('updateChefOrderCompleted', docId, newContent, companyName);
    ...
    useEffect(() => {
        ...
        socketRef.current.emit('joinChefOrderCompleted', getAppStoreData.companyName);

        // Listen for the 'menuList' event from the server
        socketRef.current.on('sendChefrOrderCompleted', (responceData) => {
